// /tutoring/ 인덱스 페이지 재생성 스크립트
// 학습플랜 가이드 - 카드 그리드 UI + 페이지네이션 버전

const fs = require('fs');
const path = require('path');

const contentDir = path.join(__dirname, '..', 'content', 'tutoring');
const outputFile = path.join(__dirname, 'tutoring-index-update.json');
...
async function main() {
  console.log('=== /tutoring/ 인덱스 페이지 재생성 (카드 그리드 UI + 페이지네이션) ===\n');
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (meta && meta.title) {
      ...
      articles.push({
        title: meta.title,
        url: `/tutoring/${slug}/`,
        description: meta.description || '학습플랜에 관한 유용한 정보를 확인하세요.',
  ...
  const kvData = [{ key: "/tutoring/index", value: html }];
  fs.writeFileSync(outputFile, JSON.stringify(kvData, null, 2));
  console.log(`\n✅ 생성 완료: ${outputFile}`);
  console.log(`npx wrangler kv bulk put "tutoring-index-update.json" --namespace-id 725aefb7b1c64c6c90d2cf4daf061bf3 --remote`);
}

function generateHtml(articles, itemListElement) {
  const title = '학습플랜 가이드 | 과외 · 학원 · 자기주도 | 과외를부탁해';
  const description = '나에게 맞는 학습 방법을 찾아보세요. 1:1 과외, 그룹 수업, 학원, 자기주도 학습까지 다양한 학습플랜을 비교하고 선택할 수 있습니다.';
  const jsonLd = { "@context": "https://schema.org", "@type": "ItemList", "name": "학습플랜 가이드",
    "description": description, "url": "https://edukoreaai.com/tutoring/", "numberOfItems": articles.length, "itemListElement": itemListElement };
...
<div class="guide-card-content"><span class="guide-card-category">학습플랜</span><h3 class="guide-card-title">${article.title}</h3>
...
<span class="guide-stat">과외 · 학원 · 자기주도</span>
</div>
</div>
