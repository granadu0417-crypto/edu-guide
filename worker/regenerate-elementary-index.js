// /elementary/ 인덱스 페이지 재생성 스크립트
// 초등학생 학습 가이드 - 카드 그리드 UI 버전

const fs = require('fs');
const path = require('path');

const contentDir = path.join(__dirname, '..', 'content', 'elementary');
const outputFile = path.join(__dirname, 'elementary-index-update.json');
...
const defaultImages = [
  ...
  'https://images.unsplash.com/photo-1588075592446-265fd1e6e76f?w=800&h=450&fit=crop',
  'https://images.unsplash.com/photo-1560785496-3c9d27877182?w=800&h=450&fit=crop',
...
// 메인 실행
async function main() {
  console.log('=== /elementary/ 인덱스 페이지 재생성 (카드 그리드 UI) ===\n');
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (meta && meta.title) {
      ...
      articles.push({
        title: meta.title,
        url: `/elementary/${slug}/`,
        description: meta.description || '초등학생 학습에 관한 유용한 정보를 확인하세요.',
  ...
  console.log(`\n✅ 생성 완료: ${outputFile}`);
  console.log(`\n다음 명령어로 KV에 업로드:`);
  console.log(`npx wrangler kv bulk put "elementary-index-update.json" --namespace-id 725aefb7b1c64c6c90d2cf4daf061bf3 --remote`);
}

function generateHtml(articles, itemListElement) {
  const title = '초등학생 학습 가이드 | 기초부터 탄탄하게 | 과외를부탁해';
  const description = '초등학생 자녀의 학습 습관을 올바르게 잡아주세요. 학년별 맞춤 학습법, 과목별 공부 방법, 독서 습관까지 꼼꼼하게 안내합니다.';

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    "name": "초등학생 학습 가이드 | 기초부터 탄탄하게",
    "description": description,
    "url": "https://edukoreaai.com/elementary/",
...
<span class="guide-card-category">초등학생</span>
...
<div class="guide-header guide-header-elementary">
<h1 class="guide-header-title">초등학생 학습 가이드</h1>
...
<span class="guide-stat">기초 · 습관 · 학습법</span>
</div>
</div>
