...
### 2026-01-11 - 배포 #15: 25개 센터 인덱스 페이지 수정 ✅

#### 문제 발견
- /coaching/eunpyeong/, /coaching/cheongra/ 등 25개 센터 인덱스 페이지가 불완전함
- 문제점:
  - 51개 학습 가이드 링크 누락
  - FAQ, 수업료 테이블 누락
  - Hugo 숏코드가 렌더링되지 않음 (raw text로 표시)
- 원인: 등록번호 수정 시 인덱스 페이지가 간소화된 형태로 재생성됨

#### 해결 방법
1. `fix-25-center-indexes.js` 스크립트 생성
2. 마포점 인덱스 페이지 형식을 기준으로 템플릿 작성
3. 51개 아티클 링크 (수학 17개 + 영어 17개 + 종합 17개)
4. 수업료 테이블 (지역 A/B 구분)
5. FAQ 섹션, 학교 정보 섹션 포함

#### 대상 센터 (25개)
eunpyeong, cheongra, rst, mjd, bd2, pungsan, jj2, jg, shd, bg, d2, ds2, gj, sch, taepyeong, jj3, bs3, gp, pst, dgj, ds3, eunpyeong2, eunpyeong3, bnj, mg

#### 배포 파일
- fix-25-center-indexes-kv.json (25개)

#### 검증
```bash
# 라이브 사이트 검증
# 은평점: 51개 아티클 링크 ✅, FAQ ✅, 수업료 ✅
# 청라점: 51개 아티클 링크 ✅, FAQ ✅, 수업료 ✅
```

#### 결과
✅ 25개 센터 인덱스 페이지 정상화 완료
