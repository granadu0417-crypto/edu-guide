// /high/ 인덱스 페이지 재생성 스크립트 (카드 그리드 버전)
// 지역 콘텐츠 제외, 일반 학습 가이드만 포함

const fs = require('fs');
const path = require('path');

const contentDir = path.join(__dirname, '..', 'content', 'high');
const outputFile = path.join(__dirname, 'high-index-update.json');
...
async function main() {
  console.log('=== /high/ 인덱스 페이지 재생성 (카드 그리드) ===\n');
  ...
  for (const file of generalGuides) {
    ...
    if (meta && meta.title) {
      ...
      const imageIndex = articles.length % defaultImages.length;

      articles.push({
        title: meta.title,
        url: `/high/${slug}/`,
        description: meta.description || '고등학생을 위한 학습 가이드입니다.',
        date: meta.date || '2025-10-28',
        image: meta.featured_image || defaultImages[imageIndex]
      });
    }
  }
  ...
  console.log(`\n✅ 생성 완료: ${outputFile}`);
  console.log(`npx wrangler kv bulk put "high-index-update.json" --namespace-id 725aefb7b1c64c6c90d2cf4daf061bf3 --remote`);
}

function generateHtml(articles, itemListElement) {
  const title = '고등학생 학습 가이드 | 내신·수능 완벽 대비 | 과외를부탁해';
  const description = '고등학교 학습의 모든 것을 담았습니다. 내신과 수능 병행 전략, 학년별 맞춤 학습법으로 대입 성공을 위한 체계적인 가이드를 제공합니다.';

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    "name": "고등학생 학습 가이드 | 내신·수능 완벽 대비",
    "description": description,
    "url": "https://edukoreaai.com/high/",
...
<span class="guide-card-category">고등학생</span>
...
<h1 class="guide-header-title">고등학생 학습 가이드</h1>
<p class="guide-header-desc">${description}</p>
<div class="guide-header-stats">
<div class="guide-stat">
<span class="guide-stat-number">${articles.length}</span>
<span class="guide-stat-label">학습 가이드</span>
</div>
<div class="guide-stat">
<span class="guide-stat-number">내신+수능</span>
<span class="guide-stat-label">완벽 대비</span>
</div>
</div>
</div>
