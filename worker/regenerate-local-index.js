// /local/ 인덱스 페이지 재생성 스크립트
// 지역별 정보 가이드 - 카드 그리드 UI + 페이지네이션 버전

const fs = require('fs');
const path = require('path');

const contentDir = path.join(__dirname, '..', 'content', 'local');
const outputFile = path.join(__dirname, 'local-index-update.json');
...
const defaultImages = [
  ...
  'https://images.unsplash.com/photo-1519677100203-a0e668c92439?w=800&h=450&fit=crop'
];
...
async function main() {
  console.log('=== /local/ 인덱스 페이지 재생성 (카드 그리드 UI + 페이지네이션) ===\n');
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (meta && meta.title) {
      ...
      articles.push({
        title: meta.title,
        url: `/local/${slug}/`,
        description: meta.description || '지역별 교육 정보를 확인하세요.',
  ...
  const kvData = [{ key: "/local/index", value: html }];
  fs.writeFileSync(outputFile, JSON.stringify(kvData, null, 2));
  console.log(`\n✅ 생성 완료: ${outputFile}`);
  console.log(`npx wrangler kv bulk put "local-index-update.json" --namespace-id 725aefb7b1c64c6c90d2cf4daf061bf3 --remote`);
}

function generateHtml(articles, itemListElement) {
  const title = '지역별 교육 정보 | 학원 · 과외 · 교육환경 | 과외를부탁해';
  const description = '전국 각 지역의 교육 환경과 학원가 정보를 확인하세요. 지역별 특화된 교육 정보와 과외 매칭 서비스를 제공합니다.';
  const jsonLd = { "@context": "https://schema.org", "@type": "ItemList", "name": "지역별 교육 정보",
    "description": description, "url": "https://edukoreaai.com/local/", "numberOfItems": articles.length, "itemListElement": itemListElement };
...
<div class="guide-card-content"><span class="guide-card-category">지역정보</span><h3 class="guide-card-title">${article.title}</h3>
...
<span class="guide-stat">학원 · 과외 · 교육환경</span>
</div>
</div>
