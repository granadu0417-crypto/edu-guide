// /consultation/ 인덱스 페이지 재생성 스크립트
// 무료 상담 가이드 - 카드 그리드 UI + 페이지네이션 버전

const fs = require('fs');
const path = require('path');

const contentDir = path.join(__dirname, '..', 'content', 'consultation');
const outputFile = path.join(__dirname, 'consultation-index-update.json');
...
const defaultImages = [
  ...
  'https://images.unsplash.com/photo-1552581234-26160f608093?w=800&h=450&fit=crop',
...
async function main() {
  console.log('=== /consultation/ 인덱스 페이지 재생성 (카드 그리드 UI + 페이지네이션) ===\n');
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (meta && meta.title) {
      ...
      articles.push({
        ...
        description: meta.description || '학습 상담에 관한 유용한 정보를 확인하세요.',
  ...
  const kvData = [{ key: "/consultation/index", value: html }];
  fs.writeFileSync(outputFile, JSON.stringify(kvData, null, 2));
  console.log(`\n✅ 생성 완료: ${outputFile}`);
  console.log(`npx wrangler kv bulk put "consultation-index-update.json" --namespace-id 725aefb7b1c64c6c90d2cf4daf061bf3 --remote`);
}

function generateHtml(articles, itemListElement) {
  const title = '무료 상담 안내 | 학습 상담 · 과외 매칭 | 과외를부탁해';
  const description = '학습 고민을 전문 상담으로 해결하세요. 학습 플랜, 과외 매칭, 학원 선택까지 무료로 상담받을 수 있습니다.';
  const jsonLd = { "@context": "https://schema.org", "@type": "ItemList", "name": "무료 상담 안내",
    "description": description, "url": "https://edukoreaai.com/consultation/", "numberOfItems": articles.length, "itemListElement": itemListElement };
...
<div class="guide-card-content"><span class="guide-card-category">상담</span><h3 class="guide-card-title">${article.title}</h3>
...
<span class="guide-stat">상담 · 매칭 · 플랜</span>
</div>
</div>
