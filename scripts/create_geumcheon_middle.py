...
# 금천구 동별 정보
GEUMCHEON_DONGS = [
    {"id": "gasan", "name": "가산동", "schools": ["가산중"]},
    {"id": "doksan1", "name": "독산1동", "schools": ["가산중", "난곡중", "문성중"]},
    {"id": "doksan2", "name": "독산2동", "schools": ["세일중", "안천중"]},
    {"id": "doksan3", "name": "독산3동", "schools": ["난곡중", "문성중"]},
    {"id": "doksan4", "name": "독산4동", "schools": ["안천중", "세일중"]},
    {"id": "siheung1", "name": "시흥1동", "schools": ["시흥중", "문일중"]},
    {"id": "siheung2", "name": "시흥2동", "schools": ["동일중", "한울중"]},
    {"id": "siheung3", "name": "시흥3동", "schools": ["시흥중", "동일중"]},
    {"id": "siheung4", "name": "시흥4동", "schools": ["문일중", "한울중"]},
    {"id": "siheung5", "name": "시흥5동", "schools": ["시흥중", "문일중", "동일중"]},
]
...
def create_geumcheon_math_content(dong_info, index):
...
"""
    ...
    content = f'''---
title: "금천구 {dong_name} 중등 수학과외 | {school_list} 내신 완벽 대비"
date: 2025-01-29
categories:
  - 중등교육
regions:
...
cities:
  - 금천구
description: "금천구 {dong_name} 중학생 수학과외 전문. {school_list} 내신 맞춤 관리. 개념부터 심화까지 체계적 1:1 지도."
tags:
  - {dong_name}
  - 금천구
  - 중등수학
...
금천구는 가산디지털단지와 인접한 주거 지역으로, 교육열이 높은 학부모님들이 많습니다. 대형 학원가는 인근 구로구나 관악구를 이용하는 경우도 있지만, 이동 시간을 줄이고 맞춤 관리를 원하는 가정에서 1:1 과외를 선호합니다.
...
금천구 {dong_name} 중학생 수학과외 수업료는 다음과 같습니다.
...
금천구 {dong_name}에서 중학생 수학과외를 찾고 계신다면, 지금 바로 상담받아보세요. {school_list} 내신에 맞춘 1:1 맞춤 수업으로 수학 실력을 확실히 올려드립니다. 수학 때문에 고민하는 시간이 길어질수록 손해입니다. 지금 시작하면 다음 시험에서 결과가 달라집니다.
'''
    return content


def create_geumcheon_english_content(dong_info, index):
    dong_name = dong_info["name"]
    schools = dong_info["schools"]
    school_list = get_school_list_text(schools)
    school_tags = get_school_tags(schools)
...
"""
    ...
    content = f'''---
title: "금천구 {dong_name} 중등 영어과외 | {school_list} 내신 완벽 대비"
date: 2025-01-29
categories:
  - 중등교육
regions:
...
cities:
  - 금천구
description: "금천구 {dong_name} 중학생 영어과외 전문. {school_list} 내신 맞춤 관리. 문법·독해·어휘 체계적 1:1 지도."
tags:
  - {dong_name}
  - 금천구
  - 중등영어
...
금천구는 가산디지털단지와 인접한 주거 지역입니다. IT 산업 종사자 가정이 많아 교육에 관심이 높고, 영어 실력의 중요성을 잘 알고 계십니다. 영어는 꾸준히 쌓아가야 하는 과목이라 일찍 시작할수록 유리합니다.
...
금천구 {dong_name} 중학생 영어과외 수업료는 다음과 같습니다.
...
금천구 {dong_name}에서 중학생 영어과외를 찾고 계신다면, 지금 바로 상담받아보세요. {school_list} 내신에 맞춘 1:1 맞춤 수업으로 영어 실력을 확실히 올려드립니다. 영어 때문에 고민하는 시간이 길어질수록 손해입니다. 지금 시작하면 다음 시험에서 결과가 달라집니다.
'''
    return content
...
def main():
    ...
    for i, dong in enumerate(GEUMCHEON_DONGS):
        # 수학 파일 생성
        math_filename = f"geumcheon-{dong['id']}-middle-math.md"
        math_filepath = os.path.join(output_dir, math_filename)
        math_content = create_geumcheon_math_content(dong, i)
        ...
        # 영어 파일 생성
        english_filename = f"geumcheon-{dong['id']}-middle-english.md"
        english_filepath = os.path.join(output_dir, english_filename)
        english_content = create_geumcheon_english_content(dong, i)

        with open(english_filepath, 'w', encoding='utf-8') as f:
            f.write(english_content)
        created_files.append(english_filename)

    print(f"금천구: {len(created_files)}개 파일 생성 완료")
