...
DISTRICTS = {
    "bupyeong": {
        "name_ko": "부평구",
        "dongs": ["부평동", "십정동", "산곡동", "청천동", "갈산동", "삼산동", "부개동", "일신동", "구산동", "부평1동"]
    },
    "namdong": {
        "name_ko": "남동구",
        "dongs": ["구월동", "간석동", "만수동", "장수동", "서창동", "운연동", "남촌동", "논현동", "고잔동", "도림동"]
    },
    "yeonsu": {
        "name_ko": "연수구",
        "dongs": ["연수동", "청학동", "옥련동", "선학동", "송도동", "동춘동", "함박뫼"]
    },
    "seo_ic": {
        "name_ko": "서구",
        "dongs": ["검암동", "경서동", "공촌동", "연희동", "가정동", "석남동", "신현동", "가좌동", "마전동", "당하동"]
    },
    "gyeyang": {
        "name_ko": "계양구",
        "dongs": ["계산동", "작전동", "효성동", "서운동", "임학동", "용종동", "병방동", "귤현동", "동양동", "박촌동"]
    },
    "michuhol": {
        "name_ko": "미추홀구",
        "dongs": ["주안동", "도화동", "숭의동", "용현동", "학익동", "문학동", "관교동", "문학동"]
    },
    "dong_ic": {
        "name_ko": "동구",
        "dongs": ["만석동", "화수동", "송현동", "창영동", "금곡동", "송림동"]
    },
    "jung_ic": {
        "name_ko": "중구",
        "dongs": ["영종동", "운서동", "중산동", "을왕동", "용유동", "신흥동", "북성동", "관동"]
    },
    "ganghwa": {
        "name_ko": "강화군",
    ...
    "ongjin": {
        "name_ko": "옹진군",
        "dongs": ["영흥면", "자월면", "덕적면", "북도면", "연평면", "백령면", "대청면"]
    },
}
...
def generate_content(district_name_ko, dong_name, subject, level, index):
    ...
    if is_math:
        intros = [
            f"""인천 {district_name_ko} {dong_name}에서 수학 때문에 고민이신가요? 문제를 많이 풀어도 실력이 늘지 않는다면 방법이 잘못된 것입니다. 수학은 개념 이해가 먼저입니다. 공식을 외우기만 해서는 응용 문제를 풀 수 없습니다.
...
수학을 잘하면 대학 선택의 폭이 넓어집니다. 문과든 이과든 수학은 중요합니다. 지금부터 준비하면 충분히 목표에 도달할 수 있습니다.""",
            f"""인천 {district_name_ko} {dong_name} 학생 여러분, 수학 성적 때문에 스트레스받고 계신가요? 중학교 때는 잘했는데 고등학교에 와서 갑자기 어려워졌다는 말, 정말 많이 듣습니다.
...
학원에서 따라가기 힘들다면 1:1 과외가 답입니다. 본인의 속도에 맞춰 공부할 수 있습니다.""",
        ...
        why_section = """1:1 과외의 가장 큰 장점은 맞춤 수업입니다. 학생의 수준에 맞춰 진도를 조절합니다. 이해가 안 되면 더 설명하고, 이해했으면 다음으로 넘어갑니다.
...
학습 습관도 잡아드립니다. 혼자 공부할 때 어떻게 해야 하는지 알려드립니다. 진정한 실력 향상은 자기주도 학습에서 나옵니다."""
        method_section = """수학 공부의 핵심은 개념 이해입니다. 공식만 외워서는 응용 문제를 풀 수 없습니다. 왜 그런 공식이 나오는지 알아야 합니다.
...
복습을 습관화하세요. 한 번 배운 내용도 안 보면 잊어버립니다. 누적 복습이 실력 향상의 비결입니다."""
        grade_section = """학년별로 전략이 다릅니다. 고1은 기초를 다지는 시기입니다. 중학교 내용에 빈틈이 있다면 먼저 메워야 합니다.
...
내신과 수능의 균형도 중요합니다. 둘 다 잘 봐야 원하는 대학에 갈 수 있습니다."""
        ...
        extra_section = ("자주 하는 실수", """학생들이 자주 하는 실수를 알려드립니다. 가장 흔한 건 계산 실수입니다. 급하게 풀다 보면 부호를 틀리거나 숫자를 잘못 쓰는 경우가 많습니다.

개념을 대충 알고 넘어가는 것도 문제입니다. 이해한 것 같아도 막상 문제에 적용하면 막힙니다.

문제를 끝까지 읽지 않는 실수도 있습니다. 조건을 놓치거나, 묻는 것과 다른 답을 쓰는 경우가 있습니다.

검산을 안 하는 것도 실수입니다. 시간이 없어서 검산을 건너뛰는 학생이 많습니다.""")
    else:  # english
        intros = [
            f"""인천 {district_name_ko} {dong_name}에서 영어 성적 때문에 고민이신가요? 단어만 외운다고 영어 실력이 늘지 않습니다. 문법을 알아야 문장을 정확히 해석할 수 있습니다.
...
영어를 잘하면 수능에서 유리합니다. 절대평가라고 해도 1등급을 받아야 경쟁력이 있습니다.""",
            f"""인천 {district_name_ko} {dong_name} 학생 여러분, 영어 공부 어떻게 해야 할지 모르겠나요? 많은 학생들이 영어를 어려워합니다.
...
영어는 언어입니다. 언어는 매일 접해야 늡니다. 매일 조금씩이라도 영어를 보고 듣고 읽어야 합니다.""",
        ...
        why_section = """1:1 과외의 장점은 맞춤 수업입니다. 학생의 현재 수준에 맞춰 시작합니다. 어휘가 부족한지, 문법이 약한지 파악합니다.
...
학습 습관도 잡아드립니다. 매일 단어 외우는 방법, 효율적인 독해 방법 등을 알려드립니다."""
        method_section = """영어 공부의 기본은 어휘입니다. 단어를 모르면 문장을 읽어도 이해할 수 없습니다. 매일 꾸준히 단어를 외우세요.
...
독해는 연습량이 중요합니다. 처음에는 느려도 괜찮습니다. 정확하게 읽는 연습을 먼저 하세요."""
        grade_section = """학년별 전략을 알려드립니다. 고1은 기초를 다지는 시기입니다. 중학교 문법에 빈틈이 있다면 먼저 메우세요.
...
EBS 연계 교재를 잘 활용하세요. 수능에 연계되는 교재입니다."""
        ...
        extra_section = ("독해 실력 향상 팁", """독해 실력을 올리는 방법을 알려드립니다. 먼저 문제를 보고 지문을 읽으세요. 무엇을 묻는지 알고 읽으면 핵심을 빠르게 찾을 수 있습니다.
...
모르는 단어에 너무 집착하지 마세요. 전체 맥락에서 의미를 추측해보세요.""")
    ...
    content = f'''---
title: "인천 {district_name_ko} {dong_name} {level_ko} {subject_ko}과외 | 내신·수능 맞춤 수업"
...
- 내신대비
- 수능대비
description: "인천 {district_name_ko} {dong_name} 지역 {level_ko} {subject_ko} 과외 전문. 학생 맞춤 1:1 수업으로 내신과 수능을 동시에 준비합니다."
...
인천 {district_name_ko} {dong_name} 지역 {level_ko} {subject_ko} 과외 수업료입니다.
...
인천 {district_name_ko} {dong_name} {level_ko} {subject_ko}, {ending}

{make_ivory_box(ivory[7])}
'''
    return content
...
def main():
    ...
    for district_id, district_info in DISTRICTS.items():
        district_path = os.path.join(BASE_PATH, district_id)
        print(f"\n=== {district_info['name_ko']} ({district_id}) ===")

        file_index = 0
        for dong in district_info["dongs"]:
            dong_path = os.path.join(district_path, dong)
            ...
            for level in ["middle", "high"]:
                for subject in ["math", "english"]:
                    content = generate_content(
                        district_info['name_ko'], dong, subject, level, file_index
                    )
